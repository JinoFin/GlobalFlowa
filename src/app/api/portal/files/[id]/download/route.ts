...
export async function GET(_request: Request, { params }: Context) {
  ...
  const { data: file } = await serviceClient.from("request_files").select("id, request_id, file_name, storage_bucket, storage_path, is_final_deliverable, customer_visible, published_at, deleted_at").eq("id", id).maybeSingle();
  ...
  return NextResponse.redirect(signed.signedUrl, { headers: privateNoStoreHeaders() });
}
