    ...
    <div className="min-h-screen bg-navy-50 lg:grid lg:grid-cols-[256px_minmax(0,1fr)]">
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-30 flex min-h-16 items-center justify-between border-b border-navy-100 bg-white px-4 lg:hidden">
          ...
          <button type="button" aria-label={`Open ${type} navigation`} aria-expanded={open} aria-controls={`${type}-mobile-navigation`} onClick={() => setOpen(true)} className="min-h-11 rounded-md border border-navy-200 px-4 text-sm font-semibold text-navy-950 outline-none focus-visible:ring-2 focus-visible:ring-teal-400">Menu</button>
        </header>
        {open ? <div className="fixed inset-0 z-50 lg:hidden"><button type="button" aria-label="Close navigation" className="absolute inset-0 bg-navy-950/40" onClick={() => setOpen(false)} /><aside id={`${type}-mobile-navigation`} role="dialog" aria-modal="true" aria-label={`${type} navigation`} className="absolute right-0 top-0 flex h-full w-[min(88vw,340px)] flex-col bg-white p-5 shadow-xl"><div className="mb-5 flex items-center justify-between"><p className="font-semibold text-navy-950">Navigation</p><button ref={closeButtonRef} type="button" onClick={() => setOpen(false)} className="min-h-11 rounded-md px-3 text-sm font-semibold outline-none focus-visible:ring-2 focus-visible:ring-teal-400">Close</button></div><Navigation items={items} pathname={pathname} onNavigate={() => setOpen(false)} /><div className="mt-5 border-t border-navy-100 pt-5">{type === "portal" ? <div className="space-y-2"><Link href="/request" onClick={() => setOpen(false)} className="flex min-h-11 items-center rounded-md bg-teal-500 px-3 py-2 text-sm font-semibold text-navy-950">Start New Request</Link><Link href="/contact" onClick={() => setOpen(false)} className="flex min-h-11 items-center rounded-md px-3 py-2 text-sm font-semibold text-navy-700">Help / Contact</Link></div> : <div className="space-y-2"><Link href="/api/admin/export" className="flex min-h-11 items-center rounded-md px-3 py-2 text-sm font-semibold text-navy-700">Export CSV</Link><Link href="/portal" className="flex min-h-11 items-center rounded-md px-3 py-2 text-sm font-semibold text-navy-700">Open Customer Portal</Link></div>}</div><div className="mt-auto border-t border-navy-100 pt-5">{logout}</div></aside></div> : null}
        {children}
      </div>
    </div>
  );
